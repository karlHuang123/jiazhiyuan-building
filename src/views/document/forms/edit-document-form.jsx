...
class EditDocumentForm extends Component {
  ...
  render() {
    const { visible, onCancel, onOk, form, fileTypeOptions, currentRowData} = this.props;
    ...
    >
        ...
        <Form.Item label="文件类型:">
            {getFieldDecorator("fileType", {
                initialValue: currentRowData ? {key: currentRowData.fileType, label: currentRowData.fileTypeName } : null,
          ...
          <Form.Item label="文件路径:" style={{height: '0', visibility: 'hidden'}}>
            {getFieldDecorator("filePath", {
              initialValue: currentRowData ? currentRowData.fileUrl : ''
            })(<Input placeholder="请输入备注" />)}
          </Form.Item>
          <div style={{visibility: this.state.filePath ? 'hidden' : 'visible', wordWrap: 'normal'}}>{currentRowData ? currentRowData.fileUrl : ''}</div>
        </Form>
      </Modal>
    );
  }
...
export default Form.create({ name: "EditDocumentForm" })(EditDocumentForm);
