...
const { Option } = Select
class AddProbsForm extends Component {
  state = {
      numOptions: [
          {label: '3', value: '3.0'},
          {label: '2', value: '2.0'},
  ...
  render() {
    ...
    getFieldDecorator('specificTermList', { initialValue: [{info: '', nth: 1, type: 'SPECIFIC_TERMS'}] });
    const specificTermList = getFieldValue('specificTermList');

    const specificTermListItems = specificTermList.map((item, index) => (
      <Form.Item key={index}>
          <div className="flex align-center">
            {index + 1}、<Input style={{width: '90%'}} placeholder="请输入规范与条款" onChange={this.handleSpecificTermListItem.bind(null, index)} />
            {index > 0 ? (
    ...
    getFieldDecorator('suggestActionList', { initialValue: [{info: '', nth: 1, type: 'SUGGEST_ACTIONS'}] });
    const suggestActionList = getFieldValue('suggestActionList');

    const suggestActionListItems = suggestActionList.map((item, index) => (
      <Form.Item key={index}>
          <div className="flex align-center">
            {index + 1}、<Input style={{width: '90%'}} placeholder="请输入建议措施" onChange={this.handleSuggestAction.bind(null, index)} />
            {index > 0 ? (
    ...
    getFieldDecorator('problemNumSetList', { initialValue: [{name: '', value: '', deductPoint: '', additionPoint: ''}] });
    ...
    getFieldDecorator('problemScaleSetList', { initialValue: [{name: '', factor: ''}] });
        ...
        <div className="grid grid-1">
          ...
          <Form.Item label="文件路径:" style={{height: '0', visibility: 'hidden'}}>
            {getFieldDecorator("incidentCase", {
              initialValue: this.state.incidentCase
...
export default Form.create({ name: "AddProbsForm" })(AddProbsForm);
